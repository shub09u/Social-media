...
import { userlogout, Userslice } from '../Store/Userslice';
  ...
  const renderMenu = (
    ...
    >
      <div className=''>
        ...
        <li onClick={''}> <Link to={'/login'}>Login</Link> </li>
        <li onClick={''}> <Link to={'/'}>Home</Link> </li>
        <li onClick={''}> <Link to={'/Signup'}>Signup</Link> </li>
        <li onClick={''}> <Link to={'/profile'}>Profile</Link> </li>
          ...
          <div className='bg-white text-black absolute top-full ml-80 w-[220px] '>
            {
              searchUser.map((ele, i) => {
                ...
                return ele._id === userslice.user?._id ? <Link state={ele._id} onClick={() => setSearchUser([])} to={'/profile'} className='flex item-center gap-4 px-2 border-b border-[#e1dbdb'>
                  ...
                  :
                  <Link onClick={() => setSearchUser([])} state={ele._id} to={`/friendprofile?name= ${ele.name} && id = ${ele._id}`} className='flex item-center gap-4 px-2 border-b border-[#e1dbdb'>
                    <img className='w-12 h-12 rounded-full border-amber-500 border' src={ele.profilePic} alt="" />
                    <h3>{ele.name}</h3>
                  </Link>
              })
